#ifndef KCS_SOLVER_HPP
#define KCS_SOLVER_HPP
...
public:
    ...
    bool update_static_vertices(const KcsVec3 *vertices,
                                uint32_t vertex_count);
    bool set_shell_mesh(const KcsVec3 *vertices, uint32_t vertex_count,
                        const KcsTriangle *triangles, uint32_t triangle_count,
                        const KcsShellMaterial &material);
    bool set_shell_seams(const KcsSeam *seams, uint32_t seam_count);
    bool build();
...
struct KcsSolver {
    explicit KcsSolver(const KcsSolverDesc &desc) : impl(desc) {}
    kcs::Solver impl;
};
