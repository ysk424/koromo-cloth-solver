...
Vec3 from_public(KcsVec3 v) { return {v.x, v.y, v.z}; }
...
bool StaticBvh::build(const std::vector<Vec3> &vertices,
                      const std::vector<KcsTriangle> &triangles,
                      std::string &error) {
    vertices_ = vertices;
    triangles_.clear();
    order_.clear();
    ...
    for (size_t ti = 0; ti < triangles.size(); ++ti) {
        const KcsTriangle t = triangles[ti];
...
Solver::Solver(const KcsSolverDesc &desc) : desc_(desc) {
...
bool Solver::set_static_mesh(const KcsVec3 *vertices, uint32_t vertex_count,
    ...
    error_.clear();
    if (built_) {
        error_ = "meshes cannot be changed after kcsBuild";
        return false;
    }
...
bool Solver::update_static_vertices(const KcsVec3 *vertices,
                                    uint32_t vertex_count) {
    error_.clear();
    if (!built_) {
        error_ = "kcsBuild must succeed before updating STATIC vertices";
        return false;
    }
...
bool Solver::set_shell_mesh(const KcsVec3 *vertices, uint32_t vertex_count,
                            const KcsTriangle *triangles,
                            uint32_t triangle_count,
...
bool Solver::set_shell_seams(const KcsSeam *seams, uint32_t seam_count) {
    error_.clear();
    if (built_) {
        error_ = "seams cannot be changed after kcsBuild";
        return false;
    }
    if (seam_count == 0u) {
        shell_seams_.clear();
    ...
    if (!shell_set_) {
        error_ = "kcsSetShellMesh must be called before kcsSetShellSeams";
        return false;
    }
    if (!seams) {
    ...
    for (const KcsSeam &seam : shell_seams_) {
...
bool Solver::build() {
    error_.clear();
    if (built_) {
        error_ = "kcsBuild may only be called once";
        return false;
    }
    if (!shell_set_) {
...
bool Solver::build_shell_constraints() {
    ...
    for (const KcsTriangle &t : shell_triangles_) {
...
bool Solver::build_strain_constraints() {
    ...
    for (const KcsTriangle &triangle : shell_triangles_) {
